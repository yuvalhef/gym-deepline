...
def _kmeans_single(X, n_clusters, x_squared_norms, max_iter=300,
    ...
    for i in range(max_iter):
        ...
        if sp.issparse(X):
            centers = _k_means._centers_sparse(X, np.ones(X.shape[0]).reshape(X.shape[0]), labels, n_clusters,
                                               distances)
        else:
            centers = _k_means._centers_dense(X, np.ones(X.shape[0]).reshape(X.shape[0]), labels, n_clusters, distances)
